...
class SimpleBBoxLabeling:
    ...
    def _draw_bbox(self, img):
        ...
        # 在图像下方多出BAR_HEIGHT这么多像素的区域用于显示文件名和当前标注物体等信息
        h, w = img.shape[:2]
        canvas = cv2.copyMakeBorder(img, 0, BAR_HEIGHT, 0, 0, cv2.BORDER_CONSTANT, value=cv2.COLOR_GRAY)
    ...
    def start(self):
        ...
        while key != KEY_ESC:
            ...
            # 删除当前图片和对应标注信息
                ...
                if messagebox.askyesno('Delete Sample', 'Are you sure?'):
...
if __name__ == '__main__':
    dir_with_images = filedialog.askdirectory()
    print(dir(dir_with_images))
